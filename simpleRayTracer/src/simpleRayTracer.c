...
int messageTag = 999;
  unsigned char *buff = (unsigned char*) calloc(3*WIDTH*HEIGHT, sizeof(char));

//MPI_Gather((img+(rank*messageN)),messageN,MPI_CHAR,buff,messageN,MPI_CHAR,0,MPI_COMM_WORLD);

if(rank != 0){
unsigned char* messageOut = img + (rank*messageN); 
MPI_Send(messageOut, messageN, MPI_CHAR, messageDest, messageTag, MPI_COMM_WORLD);
}
...
unsigned char* messageIn = img+(i*messageN);
MPI_Recv(messageIn, messageN,MPI_CHAR,i,messageTag,MPI_COMM_WORLD,&status);
}
